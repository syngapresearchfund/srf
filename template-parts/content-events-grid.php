...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'shadow-lg bg-gray-300 bg-opacity-50 xs:flex items-center ' . $col_span ); ?>>
	...
	<?php the_title( '<h2 class="entry-title w-full xs:w-4/6 px-6 text-2xl font-bold text-center xs:text-left xs:line-clamp-2 md:line-clamp-3 ' . $title_size . '"><a class="hover:underline" href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' ); ?>
	<?php if ( has_excerpt() ) : ?>
